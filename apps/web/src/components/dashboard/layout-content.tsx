	...
	useEffect(() => {
		...
		(async () => {
			try {
				...
				if (!sid) {
					const s = await createSessionRef.current.mutateAsync({ title: "The Company Onboarding" });
				...
				ob.setSessionId(sid);
				useTabStore.getState().openTab({ id: sid, title: "The Company Onboarding", type: "session", href: `/sessions/${sid}` });
			} catch (err) {
