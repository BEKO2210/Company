...
function WelcomePane({ onNext }: { onNext: () => void }) {
  return (
    <div className="flex flex-col items-center text-center gap-10">
      {/* The Company logo — large, centered */}
      <motion.div
      ...
      <div className="text-center space-y-3">
        ...
        <div className="space-y-1.5">
          ...
          <p className="text-sm text-muted-foreground/50 leading-relaxed max-w-xs mx-auto">
            The Company is designed to work with your own AI subscriptions. Here&apos;s how most people connect.
          </p>
        </div>
      </div>
      ...
      <div className="space-y-2">
        ...
        {isBillingEnabled() && (
          <div className="flex items-start gap-3 px-3 py-3 rounded-xl border border-foreground/[0.06] bg-foreground/[0.02]">
            ...
            <div className="text-left">
              <p className="text-[13px] font-medium text-foreground/80">The Company credits</p>
              <p className="text-[11px] text-foreground/40 leading-relaxed">
...
function AutoTopupPane({ onNext, onBack }: { onNext: () => void; onBack: () => void }) {
  ...
  return (
    <div className="flex flex-col items-center text-center gap-6">
      ...
      <div className="space-y-1.5">
        <h2 className="text-lg font-medium text-foreground/90">The Company Credits</h2>
        <p className="text-sm text-muted-foreground/50 leading-relaxed max-w-xs mx-auto">
      ...
      <div className="rounded-xl border border-foreground/[0.06] bg-foreground/[0.02] p-3.5 text-[12.5px] text-muted-foreground/60 leading-relaxed">
        Credits are used when your agent runs on The Company-managed models instead of your own API keys.
        ...
        <div className="space-y-1.5">
          ...
          <p className="text-sm text-muted-foreground/50 leading-relaxed max-w-xs mx-auto">
            {hasLLM
              ...
              : 'Log in with your coding subscription, paste an API key, or skip to use The Company credits.'}
          </p>
        </div>
      </div>
      ...
      <ConfigureModal open={modalOpen} onClose={() => setModalOpen(false)} title="Tool API Keys">
        <div className="space-y-4">
          <p className="text-xs text-muted-foreground/50 leading-relaxed">
            {isCloud
              ? 'These keys will override the default The Company-managed keys for these tools.'
...
export function SetupWizard({ onComplete }: { onComplete: () => void }) {
  ...
  const steps = useMemo<StepDef[]>(() => [
    ...
    ...(showBilling ? [{ label: 'The Company Credits', icon: CreditCard }] : []),
