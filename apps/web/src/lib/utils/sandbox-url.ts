...
const EXCLUDED_PORTS = new Set([
  ...
  4096, // OpenCode API (proxied by The Company Master)
  parseInt(SANDBOX_PORTS.KORTIX_MASTER, 10), // The Company Master itself
]);
...
/**
 * Regex to detect The Company Master proxy URLs: localhost:8000/proxy/{port}/...
 ...
 * from inside the sandbox (The Company Master). These are rewritten to appear as
...
export function parseLocalhostUrl(
  ...
  try {
    ...
    // Special case: localhost:8000/proxy/{port}/... (The Company Master proxy URL).
